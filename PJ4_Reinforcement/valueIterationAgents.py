    ...
    def runValueIteration(self):
        ...
        while iterations > 0:
            ...
            for state in self.mdp.getStates():
                (maxNextValue, optimalAction) = (-10000000, None)
                for currentAction in self.mdp.getPossibleActions(state):
                    currentValue = self.getQValue(state, currentAction)
                    if maxNextValue < currentValue:
                        (optimalAction, maxNextValue) = (currentAction, currentValue)
                
                if optimalAction != None:
                    updatedValues[state] = maxNextValue
